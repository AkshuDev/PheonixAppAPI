...
class BIN():
    ...
    def get_str(self) -> str:
        ...
        if self.use_pheonixapp_encoder:
            if not self.compressed and not self.hyper_compressed:
                data = HashDecoderT.Decode(self.bin_to_str(data), "Hype_Space_BIN").run()
            elif self.compressed:
                data = HashDecoderT.Decode(data, "Hype_Space_BIN").run()
            else:
                data = HashDecoderT.Decode(data, "Hype_Space").run()
        else:
    ...
    def push_str(self) -> None:
        ...
        if self.use_pheonixapp_encoder:
            if not self.hyper_compressed:
                data = HashDecoderT.Encode(self.content, "Hype_Space_BIN").run()
            else:
                data = HashDecoderT.Encode(self.content, "Hype_Space").run()
    ...
    def push_dict(self) -> None:
        ...
        if self.use_pheonixapp_Encode:
            if not self.hyper_compressed:
                data = HashDecoderT.Encode(dict_, "Hype_Space_BIN").run()
            else:
                data = HashDecoderT.Encode(dict_, "Hype_Space").run()
            if not self.compressed and not self.hyper_compressed:
                data = self.str_to_bytes(self.str_to_bin(data), "utf-16")
            else:
                data = self.str_to_bytes(data, "utf-16")
