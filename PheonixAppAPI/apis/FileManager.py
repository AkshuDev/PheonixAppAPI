...
class Large_File_Management_System():
    ...
    def Make_Bin_Encoded_File(self, index:int=0, mode:str="w") -> bool:
        ...
        try:
            with open(self.path[index], mode) as file:
                content = BIN(use_pheonixApp_encoder=True, compress=True).str_to_bytes(BIN(use_pheonixApp_encoder=True, compressed=True, content=self.content[index]).PCEMU(), "utf-16")
                file.write(content)
            return True
        except Exception as e:
            print(f"\nLarge_File_Management_System:\n{e}\n")
    ...
    def Make_BIN_Encoded_Files(self, indexes:tuple=(0, "max"), modes:list=["w"]) -> bool:
    ...
    def Read_Bin_Encoded_File(self, index:int=0, character_index:int=0) -> str:
        ...
        try:
            ...
            with open(self.path[index], "r") as file:
                file.seek(character_index)
                data = BIN(content=file.read()).bytes_to_str()
